...
def get_all_ropa_data_for_template():
    ...
    try:
        ...
        query = "SELECT * FROM הרופארCORDS ORDER BY created_at DESC"
...
def create_controller_sheet(wb, existing_data):
    ...
    columns_structure = [
        ...
        ("Transfer of personal data to third countries or international organisations (if applicable)", "third_country_transfers"),
        ("Appropriate safeguards (if applicable)", "safeguards"),
        ("Reasons For Not Adhering to Policy", "policy_adherence_reasons")
    ]

    # Add approved custom fields for Controller category just before Notes/Comments
    try:
        ...
        controller_custom_fields = custom_fields.get('Controller', [])

        for field in controller_custom_fields:
            columns_structure.append((field['field_name'], f"custom_{field['id']}"))
    except Exception as e:
        print(f"Error loading custom fields for Controller sheet: {str(e)}")

    # Add Notes/Comments as the last column
    columns_structure.append(("Notes/Comments", "notes_comments"))
    ...
    if not existing_data.empty:
        ...
        for row_idx, (_, row) in enumerate(existing_data.iterrows(), start_row):
            ...
            for col_idx, (header, db_field) in enumerate(columns_structure, 1):
                ...
                elif db_field.startswith("custom_"):
                    # Handle custom fields, map them from the row directly
                    # Assuming custom fields in the DB match the naming convention used here
...
def create_processor_sheet(wb):
    ...
    processor_columns = [
        ...
        ("Transfer of personal data to third countries or international organisations (if applicable)", "processor_third_country_transfers"),
        ("Appropriate safeguards (if applicable)", "processor_safeguards"),
        ("Safeguards & Measures", "processor_safeguards_measures")
    ]

    # Add approved custom fields for Processor category just before Notes/Comments
    try:
        from custom_tab_automation import get_approved_custom_fields_by_category
        custom_fields = get_approved_custom_fields_by_category()
        processor_custom_fields = custom_fields.get('Processor', [])

        for field in processor_custom_fields:
            processor_columns.append((field['field_name'], f"custom_{field['id']}"))
    except Exception as e:
        print(f"Error loading custom fields for Processor sheet: {str(e)}")

    # Add Notes/Comments as the last column
    processor_columns.append(("Notes/Comments", "processor_notes_comments"))
