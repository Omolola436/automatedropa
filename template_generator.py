...
def generate_ropa_template():
    ...
    footer_cell.value = "📄 Template generated by ROPA Management System | 🔒 Ensure data confidentiality when completing"
    footer_cell.font = Font(name="Calibri", size=9, color="4472C4", italic=True)
    footer_cell.alignment = Alignment(horizontal='center')
    ws.merge_cells(f'A{footer_row}:{get_column_letter(len(columns))}{footer_row}')

    # Add version number in a separate row to avoid merged cell conflict
    version_row = footer_row + 1
    version_cell = ws.cell(row=version_row, column=len(columns))
